<?php
/**
 * @link http://www.turen2.com/
 * @copyright Copyright (c) 土人开源CMS
 * @author developer qq:980522557
 */
namespace app\components;

use Yii;
use yii\helpers\ArrayHelper;

/**
 * Class ActiveRecordCompany  多公司
 * @package app\components
 */
class ActiveRecordCompany extends \yii\db\ActiveRecord
{
    //字段null值
    const DEFAULT_NULL = '-1';
    
    //顶级id为0
    const TOP_ID = 0;

    //状态
    const STATUS_INACTIVE = 0;//禁止状态
    const STATUS_ACTIVE = 1;//活动状态
    const STATUS_FINISH = 2;//完成状态
    const STATUS_DELETE = 3;//删除状态

    //删除状态
    const IS_DEL = 1;
    const IS_NOT_DEL = 0;
    
    //移动方向
    const ORDER_UP_TYPE = 'up';
    const ORDER_DOWN_TYPE = 'down';

    // 允许修改的列表
    public $follow = ['manage/messenger/update', 'manage/messenger/temp-import'];
    
    public $_admin;
    
    public function init()
    {
        parent::init();
        
        if(!Yii::$app->getUser()->getIsGuest()) {
            $this->_admin = Yii::$app->getUser()->getIdentity();
        }
    }
    
    public function getAdmin()
    {
        return $this->_admin;
    }

    //保存之前处理
    public function beforeSave($insert)
    {
        if (parent::beforeSave($insert)) {
//            if ($this->getAdmin()->company_id == 1) {
//                return true;
//            }
//            echo "<PRE>";print_r(Yii::$app->requestedRoute);die();
//            echo "<PRE>";print_r(Yii::$app->request->post());die();
            if($this->company_id > 0 || in_array(Yii::$app->requestedRoute, $this->follow)){
                return true;
            }

            //增加对应的公司id
            $companyId = Yii::$app->getUser()->getIdentity()->company_id;
            $this->company_id=$companyId;
            return true;
        } else {
            return false;
        }
    }
    
//    /*
//     * 获取$parentStr的第二位id
//     *
//     * @access public
//     * @param  $str    string  要拆分的整型序列如1,2,3
//     * @param  $$i     int     为空返回str数组的第二位(第一位为0)
//     * @return $topid  int     str的第一位
//     */
//    public function getTopID($parentStr = 'parentstr', $i = 1)
//    {
//        if(isset($this->{$parentStr})) {
//            if($this->{$parentStr} == '0,') {
//                $topid = 0;
//            } else {
//                $ids = explode(',', $this->{$parentStr});
//                $topid = isset($ids[$i]) ? $ids[$i] : '';
//            }
//
//            return $topid;
//        }
//
//        return null;
//    }
//
//    /**
//     * 构建parent str
//     * @return string
//     */
//    public function initParentStr($primayKey = 'id', $parentKey = 'parentid', $parentStr = 'parentstr')
//    {
//        $parentModel = self::find()->andWhere([$primayKey => $this->{$parentKey}])->one();
//        if($parentModel) {
//            if($parentModel->{$primayKey} == self::TOP_ID) {
//                $parentstr = '0,';
//            } else {
//                $parentstr = ($parentModel->{$parentStr}).($parentModel->{$primayKey}).',';
//            }
//        } else {
//            $parentstr = '0,';
//        }
//
//        return $parentstr;
//    }
}